...
separate from the normal ROM build. An optional playable build translates Birch and twelve opening scenes in Russian and German,
...
`opening.json` currently holds twelve source-symbol mappings,
...
buffer, and uses the native shadowed font for page arrows. Semicolons are
normalized to commas for display. German noun entries can use `dictionary_form`
